...
/* ===== CLEAR & ARCHIVE FUNCTION ===== */
/* 
  Securely moves all votes from "votes" collection to "archive" collection.
  Requires admin password verification before proceeding.
  Uses Firestore batching for atomic transactions.
*/
async function clearAndArchive(){
  try {
    // STEP 1: PROMPT FOR ADMIN PASSWORD
    const password = prompt("🔐 Enter admin password to confirm archiving:");
    ...
    if(password !== ADMIN_PASS) {
      alert("❌ Invalid password. Archive operation cancelled.");
      console.error("Invalid password attempt for archive operation");
      return;
    }
    
    // STEP 3: CONFIRM FINAL ACTION
    const finalConfirm = confirm(
      "⚠️  WARNING: This will permanently move all votes to archive and clear the votes collection.\n\n" +
      "This action CANNOT be undone.\n\n" +
      "Click OK to proceed, or Cancel to go back."
    );
    
    if(!finalConfirm) {
      console.log("Archive operation cancelled by user at confirmation");
      return;
    }
    ...
    // STEP 5: FETCH ALL DOCUMENTS FROM "votes" COLLECTION
    const votesSnapshot = await db.collection("votes").get();
    
    if(votesSnapshot.empty) {
      alert("ℹ️  No votes to archive. Votes collection is already empty.");
      console.log("Archive attempted but votes collection is empty");
      return;
    }
    
    // STEP 6: MOVE DOCUMENTS TO ARCHIVE USING BATCH WRITE
    ...
    votesSnapshot.forEach(doc => {
      ...
      const archiveRef = db.collection("archive").doc(doc.id);
      
      // Add document to archive collection
      batch.set(archiveRef, {
        ...voteData,
        archivedAt: new Date().toISOString() // Add timestamp when archived
      });
      
      // Delete from active votes collection
      batch.delete(doc.ref);
      movedCount++;
    });
    
    // STEP 7: COMMIT THE BATCH OPERATION
    await batch.commit();
    console.log(`✅ Successfully archived ${movedCount} vote(s)`);
    
    // STEP 8: UPDATE LOCAL ARCHIVE BACKUP
    // Keep a local copy as additional backup
    archive.push(...votesSnapshot.docs.map(doc => ({
      user: doc.id,
      ...doc.data(),
    ...
    // STEP 9: REFRESH ADMIN DASHBOARD
    loadAdmin();
    
    // STEP 10: NOTIFY ADMIN OF SUCCESS
    alert(`✅ SUCCESS!\n\n${movedCount} vote(s) have been archived and removed from active records.\nArchive collection updated.`);
    
  } catch(error) {
    // ERROR HANDLING
    console.error("❌ Archive operation failed:", error);
    alert("❌ Error during archive operation:\n\n" + error.message + "\n\nPlease check the console for details.");
  }
}
...
function printCandidates() {
  ...
  if (!printWindow) {
    return alert("Pop-up blocked. Please allow pop-ups to print candidates.");
  }

  const schoolName = "Concepcion National High School";
  const now = new Date();
  const dateString = now.toLocaleString();

  let html = `<!DOCTYPE html><html><head><meta charset="UTF-8"><title>${schoolName} Candidates</title><style>
    body { font-family: 'Courier New', Courier, monospace; color:#000; background:#fff; margin:0; padding:10px; }
    .receipt { max-width:360px; margin:0 auto; }
    .title { text-align:center; font-size:1.1rem; font-weight:700; margin:0; padding:4px 0; }
    .subtext { text-align:center; font-size:0.75rem; margin:3px 0 9px 0; }
    .section { margin-top:12px; border-top:1px dashed #000; padding-top:10px; }
    .position { font-weight:700; font-size:0.95rem; margin-bottom:4px; }
    .candidate { margin-left:8px; font-size:0.9rem; line-height:1.35; }
    .footer { text-align:center; font-size:0.72rem; margin:12px 0 0; }
    @media print {
    ...
    <div class="subtext">All Positions & Candidates</div>
    <div class="subtext">${escapeHtml(dateString)}</div>`;

  positions.forEach((positionName) => {
    const bucket = candidates[positionName] || [];
    html += `<div class="section"><div class="position">${escapeHtml(positionName)}</div>`;
    if (bucket.length === 0) {
      html += `<div class="candidate">- No candidates -</div>`;
    } else {
      bucket.forEach((candidate, index) => {
        const candidateName = candidate && candidate.name ? candidate.name : "(Unnamed)";
        html += `<div class="candidate">${index + 1}. ${escapeHtml(candidateName)}</div>`;
      });
    }
    html += `</div>`;
  });

  html += `<div class="footer">Generated from Digital Voting System (receipt style)</div></div>
  <script>window.onload=function(){setTimeout(function(){window.print();},150); window.onafterprint=function(){window.close();};};</script></body></html>`;
...
function printReceipt() {
  const receiptElement = document.getElementById("printableVotes");
  if (!receiptElement) {
    return alert("Receipt section not found.");
  }

  const receiptContent = receiptElement.innerHTML;
  const contentHtml = `
    <div class="section">
      <div class="position">Voting Receipt</div>
      ${receiptContent}
    </div>
  `;

  const doc = buildReceiptDocument("Voting Receipt", contentHtml);
  openPrintWindow(doc);
}
...
function openPrintWindow(html) {
  const printWindow = window.open("", "_blank", "width=700,height=900,scrollbars=yes");
  if (!printWindow) {
    return alert("Pop-up blocked. Please allow pop-ups to print.");
  }
  printWindow.document.open();
  printWindow.document.write(html);
  printWindow.document.close();
...
function buildReceiptDocument(reportTitle, contentHtml) {
  const schoolName = "Concepcion National High School";
  const generatedAt = new Date().toLocaleString();

  return `<!DOCTYPE html><html><head><meta charset="UTF-8"><title>${escapeHtml(
    schoolName
  )} - ${escapeHtml(reportTitle)}</title><style>
    body { font-family:'Courier New', Courier, monospace; color:#000; background:#fff; margin:0; padding:10px; }
    .receipt { max-width:360px; margin:0 auto; }
    .title { text-align:center; font-size:1.1rem; font-weight:700; margin:0; padding:4px 0; }
    .subtext { text-align:center; font-size:0.75rem; margin:3px 0 9px 0; }
    ...
    <div class="title">${escapeHtml(schoolName)}</div>
    <div class="subtext">${escapeHtml(reportTitle)}</div>
    <div class="subtext">${escapeHtml(generatedAt)}</div>
    ${contentHtml}
    <div class="footer">Generated from Digital Voting System (receipt style)</div>
  </div>
...
function printCandidates() {
  ...
  positions.forEach((positionName) => {
    const bucket = candidates[positionName] || [];
    content += `<div class="section"><div class="position">${escapeHtml(positionName)}</div>`;
    if (bucket.length === 0) {
      content += `<div class="candidate">- No candidates -</div>`;
    } else {
      bucket.forEach((candidate, index) => {
        const candidateName = candidate && candidate.name ? candidate.name : "(Unnamed)";
        content += `<div class="candidate">${index + 1}. ${escapeHtml(candidateName)}</div>`;
      });
    }
    content += `</div>`;
  });

  openPrintWindow(buildReceiptDocument("All Positions & Candidates", content));
}
...
async function printTotals() {
  try {
    ...
    positions.forEach((p) => {
      totals[p] = {};
      const positionCandidates = candidates[p] || [];
      if (positionCandidates.length > 0) {
        positionCandidates.forEach((c) => {
          const name = c && c.name ? c.name : "(Unnamed)";
          totals[p][name] = 0;
        });
      }
    });
    ...
    snapshot.forEach((doc) => {
      const data = doc.data() || {};
      const voteMap = data.votes || {};
      for (let pos in voteMap) {
        const candidateName = voteMap[pos] || "(Unknown)";
        if (!totals[pos]) totals[pos] = {};
        totals[pos][candidateName] = (totals[pos][candidateName] || 0) + 1;
      }
    });

    let content = "";
    positions.forEach((positionName) => {
      const byCandidate = totals[positionName] || {};
      content += `<div class="section"><div class="position">${escapeHtml(positionName)}</div>`;
      if (Object.keys(byCandidate).length === 0) {
        content += `<div class="candidate">- No votes yet -</div>`;
      } else {
        Object.keys(byCandidate).forEach((candidateName) => {
          const count = byCandidate[candidateName];
          content += `<div class="candidate">${escapeHtml(candidateName)}: ${count}</div>`;
        });
      }
      content += `</div>`;
    });

    openPrintWindow(buildReceiptDocument("Vote Totals", content));
  } catch (error) {
    console.error("Error printing totals:", error);
    alert("Unable to generate totals printout.");
  }
}

async function printRecords() {
  try {
    const snapshot = await db.collection("votes").orderBy("time", "desc").get();
    let content = "";

    if (snapshot.empty) {
      content = `<div class="section"><div class="candidate">No active records found.</div></div>`;
    } else {
      snapshot.forEach((doc) => {
        const data = doc.data() || {};
        const when = data.time ? formatTime(data.time) : "-";
        const email = data.email || "-";
        const name = data.name || "-";
        const lrn = data.lrn || doc.id || "-";

        content += `<div class="section"><div class="position">Record: ${escapeHtml(lrn)}</div>`;
        content += `<div class="candidate">Name: ${escapeHtml(name)}</div>`;
        content += `<div class="candidate">Email: ${escapeHtml(email)}</div>`;
        content += `<div class="candidate">Time: ${escapeHtml(when)}</div>`;

        const voteData = data.votes || {};
        if (Object.keys(voteData).length === 0) {
          content += `<div class="candidate">Votes: none</div>`;
        } else {
          content += `<div class="candidate" style="font-weight:700;">Votes:</div>`;
          Object.keys(voteData).forEach((pos) => {
            content += `<div class="candidate">${escapeHtml(pos)}: ${escapeHtml(voteData[pos])}</div>`;
          });
        }
    ...
    openPrintWindow(buildReceiptDocument("Active Vote Records", content));
  } catch (error) {
    console.error("Error printing records:", error);
    alert("Unable to generate records printout.");
  }
}

async function printArchive() {
  try {
    const snapshot = await db.collection("archive").orderBy("archivedAt", "desc").get();
    let content = "";

    if (snapshot.empty) {
      content = `<div class="section"><div class="candidate">No archived records found.</div></div>`;
    } else {
      snapshot.forEach((doc) => {
        const data = doc.data() || {};
        const when = data.archivedAt || "-";
        const voteWhen = data.time ? formatTime(data.time) : "-";
        const email = data.email || "-";
        const name = data.name || "-";
        const lrn = data.lrn || doc.id || "-";

        content += `<div class="section"><div class="position">Archived Record: ${escapeHtml(lrn)}</div>`;
        content += `<div class="candidate">Name: ${escapeHtml(name)}</div>`;
        content += `<div class="candidate">Email: ${escapeHtml(email)}</div>`;
        content += `<div class="candidate">Submitted: ${escapeHtml(voteWhen)}</div>`;
        content += `<div class="candidate">Archived: ${escapeHtml(when)}</div>`;
    ...
    openPrintWindow(buildReceiptDocument("Archived Records", content));
  } catch (error) {
    console.error("Error printing archive:", error);
    alert("Unable to generate archive printout.");
  }
}
